...
- `iex -S mix`
  - Automatically starts your application and all of its dependencies with are also applications
  - `application` function inside `mix.exs` file
    - used to start applications included or chihpped with Elixir/Erlang
  - **Erlang Applications**
    - An specification to define an Application
      - what *modules* composes the app
      - whats the *version*
      - a *description*
      - the *extra apps*
      - ...
    - Automatically generated by `mix`
  - Starting and Stopping an appliaction
    - `Application.start` and `Application.stop`
  - An **OTP Application** is a *first class entity* that can be started and stopped as a unit
    - can be *reused* in another systems
    - It's more like a *component* or a *service* than a traditional application
  - How we tell OTP to do something specific to our application?
    - we use a **callback** as always
    - as a *convention* we will use a top level module defined with the same name of the application as the callback module to OTP
    - Also, we will use a *Behavior*: `Supervisor`
    - We need to create a function `start/2` receiving *type* and *args* as the function args
      - This function will be invoked when the application starts
    - With the callback module we override the `start/2` function. Now, we need to make sure the callback module is called when the application starts. We d othis in the `mix.exs` file
      - inside the application function add another key, `mod` and give it a *tuple* with the callback module name and the *args* for the `start/2` funtion
  - the app will start automatically when we execute `iex -S mix
    - If we execute `mix run` it will execute the app and exit BEAM
    - To keep the app running we need to run `mix run --no-halt`
  -  Defining *env vars* to the application
     -  inside `mix.exs`, in the function applcaition, add the key `env` and passa a list of key-value pairs to define the list of environment vars
     -  To get the value of a *env var* we use `Applciation.get_env(app_name_atom, var_name_atom)`
     -  Another option to define an *env var* si to use a mix config file
        -  `config/config.exs`
        -  `config :my_app, :my_env_var, 100`
        -  But this approach has a problem
           -  It will not work when the application is loaded inside another one
           -  This config file will not be loaded
           -  Changes to the config files are restricted to the project itself
        -  So the config files are *project specific* configurations, while the definitions inside `mix.exs` are being *application-specific*
-  Creating a project with a supervisor
  - `mix new my_app --sup`
    - It will create an Application callback module with default configs and a *module-less* supervisor
